...
# 3. Crear el ServiceAccount que será utilizado por el CronJob
echo "3. Creando ServiceAccount 'spot-mover'..."
kubectl create serviceaccount spot-mover -n monitoring --dry-run=client -o yaml | kubectl apply -f -
echo "   ServiceAccount creado."

# 4. Crear y aplicar el RBAC de DEPURACIÓN (para superar el problema de permisos)
# Este es un RBAC de depuración que otorga amplios permisos, solo para la demo en Killercode.
echo "4. Creando y aplicando RBAC de DEPURACIÓN (¡ADVERTENCIA: SOLO PARA DEMO!)..."
...
# 5. Añadir repositorio de Helm e instalar kube-prometheus-stack
echo "5. Añadiendo repositorio de Helm e instalando 'kube-prometheus-stack'..."
...
# 6. Crear y aplicar el CronJob (configurado para cada minuto para la demo)
echo "6. Creando y aplicando el CronJob 'spot-mover-cron' (cada minuto para demo)..."
kubectl apply -f cronjob.yaml
echo "   CronJob 'spot-mover-cron' aplicado."

# 7. Crear y aplicar los Deployments de prueba (servicios bancarios)
echo "7. Creando y aplicando los Deployments de prueba 'banco'..."
...
echo "8. Verificando el parcheo del Deployment 'servicio-bancario'..."
